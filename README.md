...
```bash
# 백엔드 API Base URL (기본값은 next.config.mjs에 따라 http://localhost:3001/api)
# 백엔드가 8080에서 동작한다면 아래와 같이 맞춰주세요.
...
주의: 기본값은 `http://localhost:3001/api`이므로, 백엔드가 `8080`에서 실행된다면 반드시 `.env.local`에서 `NEXT_PUBLIC_API_BASE_URL`을 `http://localhost:8080/api`로 변경하세요.
...
### API 연동 및 주의사항
- 기본 Base URL은 `next.config.mjs`에서 `process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:3001/api'`로 설정됩니다.
- 백엔드 기본 포트는 8080이므로, 로컬 통합 시 `.env.local`에서 `NEXT_PUBLIC_API_BASE_URL=http://localhost:8080/api`로 맞추세요.
...
### 트러블슈팅
- **백엔드 연결 안 됨**: `.env.local`의 `NEXT_PUBLIC_API_BASE_URL`이 백엔드 포트와 일치하는지 확인(예: `http://localhost:8080/api`).
